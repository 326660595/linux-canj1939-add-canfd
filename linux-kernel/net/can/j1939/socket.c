			    ...
			    size_t size, int flags)
{
	...
	struct j1939_sk_buff_cb *skcb;
	int ret = 0;
	// printk(KERN_ERR "russell：j1939_sk_recvmsg------size%d\n",size);
...
static struct sk_buff *j1939_sk_alloc_skb_fd(struct net_device *ndev,
                                             struct sock *sk,
                                             struct msghdr *msg, size_t size,
                                             int *errcode)
{
	...
	// printk(KERN_ERR "russell------ msg_flags:%d\n",msg->msg_flags);
	skb = sock_alloc_send_skb(sk,
				  size +
				  CANFD_MTU -
				  sizeof(((struct canfd_frame *)NULL)->data)+
				  sizeof(struct can_skb_priv),
				  msg->msg_flags & MSG_DONTWAIT, &ret);
	if (!skb)
		goto failure;
	...
	// 检查头部空间是否足够
    // pr_err("russell------j1939:canfd_frame size %d- skb headroom-2-%d-\n",sizeof(struct canfd_frame),skb_headroom(skb));
...

{
	...
	while (todo_size) {
		...
		segment_size = min_t(size_t, J1939_MAX_TP_PACKET_SIZE,
				     todo_size);
		// printk(KERN_ERR "russell------------todo_size:%d\n",todo_size);
		//russell:增加对can fd包的处理
		// if (todo_size > 8)
		// 	skb = j1939_sk_alloc_skb_fd(priv->ndev, sk, msg, segment_size,&ret);	
		// else
		...
		if (!session) {
			...
			if (can_name)
				...
				session = j1939_tp_send_fd(priv, skb, size);
