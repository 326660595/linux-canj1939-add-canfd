...
static struct
sk_buff *j1939_tp_tx_dat_new_fd(struct j1939_priv *priv,
			     const struct j1939_sk_buff_cb *re_skcb,
			     bool ctl,
			     bool swap_src_dst)
{
	...
	skb = alloc_skb(sizeof(struct canfd_frame) + sizeof(struct can_skb_priv),
			GFP_ATOMIC);
	if (unlikely(!skb))
		return ERR_PTR(-ENOMEM);
	...
	can_skb_prv(skb)->skbcnt = 0;
	/* reserve CAN header */
	skb_reserve(skb, offsetof(struct canfd_frame, data));
...
/*fd TP transmit packet functions */
static int j1939_tp_tx_dat_fd(struct j1939_session *session,
			   const u8 *dat, int len)
{
	struct j1939_priv *priv = session->priv;
	struct sk_buff *skb;
	...
	// skb = j1939_tp_tx_dat_new(priv, &session->skcb,
	// 			  false, false);
	skb = j1939_tp_tx_dat_new_fd(priv, &session->skcb,
				  false, false);
	if (IS_ERR(skb))
		return PTR_ERR(skb);
	...
	// if (j1939_tp_padding && len < 8)
	// 	memset(skb_put(skb, 8 - len), 0xff, 8 - len);
	if (j1939_tp_padding && len < 64)
		memset(skb_put(skb, 64 - len), 0xff, 64 - len);

	// return j1939_send_one(priv, skb);
	return j1939_send_one_fd(priv, skb);
}
...
static int j1939_xtp_do_tx_ctl_fd(struct j1939_priv *priv,
			       const struct j1939_sk_buff_cb *re_skcb,
			       bool swap_src_dst, pgn_t pgn, const u8 *dat)
{
	struct sk_buff *skb;
	...
	// skb = j1939_tp_tx_dat_new(priv, re_skcb, true, swap_src_dst);
	//russell:这里改为支持fd的
	skb = j1939_tp_tx_dat_new_fd(priv, re_skcb, true, swap_src_dst);
	if (IS_ERR(skb))
		return PTR_ERR(skb);
	...
	skdat[6] = (pgn >> 8);
	skdat[7] = (pgn >> 16);
	// printk(KERN_ERR "russell---j1939_xtp_do_tx_ctl_fd--skb-len:%d-\n",skb->len);
	//return j1939_send_one(priv, skb);
	return j1939_send_one_fd(priv, skb);
}
				  ...
				  bool swap_src_dst, const u8 *dat)
{
	...
	return j1939_xtp_do_tx_ctl_fd(priv, &session->skcb,
					swap_src_dst,
					session->skcb.addr.pgn, dat);
...
static int j1939_session_tx_rts(struct j1939_session *session)
{
	...
	if (session->skcb.addr.type == J1939_ETP) {
		...
		// printk(KERN_ERR "russell-J1939_TP_CMD_BAM-----------\n");
		dat[0] = J1939_TP_CMD_BAM;
		/* fake cts for broadcast */
		session->pkt.tx = 0;
	} else {
		// printk(KERN_ERR "russell-J1939_TP_CMD_RTS-----------\n");
...
static int j1939_session_tx_dat_fd(struct j1939_session *session)
{
	struct j1939_priv *priv = session->priv;
	struct j1939_sk_buff_cb *se_skcb;
	int offset, pkt_done, pkt_end;
	...
	// u8 dat[8];
	u8 dat[64];
	//russell:can 8字节*7，canfd *63
	// se_skb = j1939_session_skb_get_by_offset(session, session->pkt.tx * 7);
	se_skb = j1939_session_skb_get_by_offset(session, session->pkt.tx * 63);
	if (!se_skb)
		return -ENOBUFS;
	...
	if (session->skcb.addr.type != J1939_ETP &&
		...
		pkt_end = session->pkt.last;
	// printk(KERN_ERR "russell---pkt_end:%d，se_skcb->offset;%d\n",pkt_end,se_skcb->offset);
	while (session->pkt.tx < pkt_end) {
		dat[0] = session->pkt.tx - session->pkt.dpo + 1;
		// offset = (session->pkt.tx * 7) - se_skcb->offset;
		offset = (session->pkt.tx * 63) - se_skcb->offset;
		len =  se_skb->len - offset;
		// if (len > 7)
		// 	len = 7;
		if (len > 63)
		...
		memcpy(&dat[1], &tpdat[offset], len);
		ret = j1939_tp_tx_dat_fd(session, dat, len + 1);
		if (ret < 0) {
			/* ENOBUFS == CAN interface TX queue is full */
			if (ret != -ENOBUFS)
				netdev_alert(priv->ndev,
		...
		// printk(KERN_ERR "russell---session->pkt.tx:%d,len:%d\n",pkt_end,len);
		session->last_txcmd = 0xff;
		pkt_done++;
		session->pkt.tx++;
		pdelay = j1939_cb_is_broadcast(&session->skcb) ? 50 :
...
static int j1939_xtp_txnext_transmiter(struct j1939_session *session)
{
	struct j1939_priv *priv = session->priv;
	int ret = 0;
	printk(KERN_ERR "russell-session->last_cmd-----%d---\n",session->last_cmd);
...
static int j1939_xtp_txnext_transmiter_fd(struct j1939_session *session)
{
	struct j1939_priv *priv = session->priv;
	int ret = 0;
	// printk(KERN_ERR "russell-session->last_cmd-----%d---\n",session->last_cmd);
	...
	case J1939_ETP_CMD_DPO:
	case J1939_TP_CMD_BAM:
		ret = j1939_session_tx_dat_fd(session);
...
static int j1939_simple_txnext(struct j1939_session *session)
{
	...
	struct sk_buff *skb;
	int ret;
	printk(KERN_ERR "russell-j1939_simple_txnext------------------\n");
...
static int j1939_simple_txnext_fd(struct j1939_session *session)
{
	struct j1939_priv *priv = session->priv;
	struct sk_buff *se_skb = j1939_session_skb_get(session);
	struct sk_buff *skb;
	int ret;
	printk(KERN_ERR "russell-j1939_simple_txnext fd------------------\n");
	if (!se_skb)
		return 0;
	...
	j1939_tp_set_rxtimeout(session, J1939_SIMPLE_ECHO_TIMEOUT_MS);
	// printk(KERN_ERR "russell------session size -%d-,skb_headroom=%d\n",session->total_message_size,skb_headroom(skb));

	ret = j1939_send_one_fd(priv, skb);
...
static enum hrtimer_restart j1939_tp_txtimer(struct hrtimer *hrtimer)
{
	...
	struct j1939_priv *priv = session->priv;
	int ret = 0;
	// printk(KERN_ERR "russell-j1939_tp_txtimer------------------\n");
...
static enum hrtimer_restart j1939_tp_txtimer_fd(struct hrtimer *hrtimer)
{
	struct j1939_session *session =
		container_of(hrtimer, struct j1939_session, txtimer);
	struct j1939_priv *priv = session->priv;
	...
	if (session->skcb.addr.type == J1939_SIMPLE) {
		// printk(KERN_ERR "russell-j1939_tp_txtimer----j1939_simple_txnextfd----------\n");
		ret = j1939_simple_txnext_fd(session);
	} else {
		// printk(KERN_ERR "russell-session->transmission---------%d-----\n",session->transmission);
		if (session->transmission)
			ret = j1939_xtp_txnext_transmiter_fd(session);
		else
			ret = j1939_xtp_txnext_receiver(session);
	}
...
static enum hrtimer_restart j1939_tp_rxtimer(struct hrtimer *hrtimer)
{
	...
	// pr_err("russell:1939_tp_rxtimer-----\n");
...
static struct j1939_session *j1939_session_new_fd(struct j1939_priv *priv,
					       struct sk_buff *skb, size_t size)
{
	struct j1939_session *session;
	struct j1939_sk_buff_cb *skcb;
	printk(KERN_ERR "russell-j1939_session_new fd-------------%d--\n",size);
	...
	hrtimer_init(&session->txtimer, CLOCK_MONOTONIC,
		     HRTIMER_MODE_REL_SOFT);
	session->txtimer.function = j1939_tp_txtimer_fd;
	hrtimer_init(&session->rxtimer, CLOCK_MONOTONIC,
		     HRTIMER_MODE_REL_SOFT);
	session->rxtimer.function = j1939_tp_rxtimer;
...
static void j1939_xtp_rx_dat_one_fd(struct j1939_session *session,
				 struct sk_buff *skb)
{
	enum j1939_xtp_abort abort = J1939_XTP_ABORT_FAULT;
	struct j1939_priv *priv = session->priv;
	...
	// printk(KERN_ERR "russell:j1939_xtp_rx_dat_one----skb->len：%d----session->last_cmd:%d---\n",skb->len,session->last_cmd);
	...
	// se_skb = j1939_session_skb_get_by_offset(session, packet * 7);
	se_skb = j1939_session_skb_get_by_offset(session, packet * 63);
	if (!se_skb) {
		netdev_warn(priv->ndev, "%s: 0x%p: no skb found\n", __func__,
			    session);
		goto out_session_cancel;
	...
	// if (nbytes > 7)
	// 	nbytes = 7;
	offset = packet * 63 - se_skcb->offset;
	nbytes = se_skb->len - offset;
	if (nbytes > 63)
		nbytes = 63;
	if (nbytes <= 0 || (nbytes + 1) > skb->len) {
...
static void j1939_xtp_rx_dat_fd(struct j1939_priv *priv, struct sk_buff *skb)
{
	struct j1939_sk_buff_cb *skcb;
	struct j1939_session *session;
	// printk(KERN_ERR "russell:j1939_xtp_rx_dat-----------\n");
	skcb = j1939_skb_to_cb(skb);

	if (j1939_tp_im_transmitter(skcb)) {
		// printk(KERN_ERR "russell:j1939_tp_im_transmitter-----------\n");
		session = j1939_session_get_by_addr(priv, &skcb->addr, false,
						    true);
		if (!session)
			netdev_info(priv->ndev, "%s: no tx connection found\n",
			...
			j1939_xtp_rx_dat_one_fd(session, skb);
	}

	if (j1939_tp_im_receiver(skcb)) {
		// printk(KERN_ERR "russell:j1939_tp_im_receiver-----------\n");
		session = j1939_session_get_by_addr(priv, &skcb->addr, false,
						    false);
		if (!session)
			netdev_info(priv->ndev, "%s: no rx connection found\n",
	...
	if (j1939_cb_is_broadcast(skcb)) {
		// printk(KERN_ERR "russell:j1939_cb_is_broadcast-----------\n");
		session = j1939_session_get_by_addr(priv, &skcb->addr, false,
						    false);
		if (session)
			j1939_xtp_rx_dat_one_fd(session, skb);
				    ...
				    struct sk_buff *skb, size_t size)
{
	...
	struct j1939_session *session;
	int ret;
	printk(KERN_ERR "russell-j1939_tp_send------------------\n");
...
/* j1939 main intf can fd */
struct j1939_session *j1939_tp_send_fd(struct j1939_priv *priv,
				    struct sk_buff *skb, size_t size)
{
	struct j1939_sk_buff_cb *skcb = j1939_skb_to_cb(skb);
	struct j1939_session *session;
	int ret;
	printk(KERN_ERR "russell-j1939_tp_send fd------------------\n");
	...
	if (size <= 8)
		skcb->addr.type = J1939_SIMPLE;
	else if (size <= 64)//russell:在simple类型增加对64字节数据的支持
		skcb->addr.type = J1939_SIMPLE;
	else if (size > J1939_MAX_TP_PACKET_SIZE)
	// else if (size > 120)
		skcb->addr.type = J1939_ETP;
	...
	if (skcb->addr.type == J1939_ETP &&
	    j1939_cb_is_broadcast(skcb))
		return ERR_PTR(-EDESTADDRREQ);
	// printk(KERN_ERR "russell-skcb->addr.type----------------%d--\n",skcb->addr.type);
	/* fill in addresses from names */
	ret = j1939_ac_fixup(priv, skb);
	if (unlikely(ret))
		return ERR_PTR(ret);
	...
	/* prepare new session */
	session = j1939_session_new_fd(priv, skb, size);
	if (!session)
		return ERR_PTR(-ENOMEM);
	...
	session->transmission = true;
	// session->pkt.total = (size + 6) / 7;
	//russell：这里得到多包传输的包数量，改为fd
	session->pkt.total = (size + 62) / 63;
	session->pkt.block = skcb->addr.type == J1939_ETP ? 255 :
		min(j1939_tp_block ?: 255, session->pkt.total);
...
int j1939_tp_recv_fd(struct j1939_priv *priv, struct sk_buff *skb)
{
	struct j1939_sk_buff_cb *skcb = j1939_skb_to_cb(skb);
	// printk(KERN_ERR "russell:j1939_tp_recv----------%x-\n",skcb->addr.pgn);
	if (!j1939_tp_im_involved_anydir(skcb) && !j1939_cb_is_broadcast(skcb))
		return 0;
