...
#define J1939_CANFD_HDR (offsetof(struct canfd_frame, data))

/* CAN_FTR: #bytes beyond data part */
#define J1939_CAN_FTR (sizeof(struct can_frame) - J1939_CAN_HDR - \
		 sizeof(((struct can_frame *)0)->data))

/* CANFD_FTR: #bytes beyond data part */
#define J1939_CANFD_FTR (sizeof(struct canfd_frame) - J1939_CANFD_HDR - \
		 sizeof(((struct canfd_frame *)0)->data))
...
static void j1939_can_recv(struct sk_buff *iskb, void *data)
{
	...
	int can_type = 0;//1 is can,2 is canfd

	// printk(KERN_ERR "russell-j1939_can_recv-----------\n");
	/* make sure we only get Classical CAN frames */
	// if (can_is_canxl_skb(iskb)) {
	// 	printk(KERN_ERR "russell--can_is_canxl_skb-----------\n");
	// } else if (can_is_can_skb(iskb)) {
	// 	printk(KERN_ERR "russell--can_is_can_skb-----------\n");
	// } else if (can_is_canfd_skb(iskb)) {
	// 	printk(KERN_ERR "russell--can_is_canfd_skb-----------\n");
	// } else {
	// 	printk(KERN_ERR "russell--inval_skb-----------\n");
	// }
	...
	{
		cfd = (void *)skb->data;
		skb_pull(skb, J1939_CANFD_HDR);
		/* fix length, set to dlc, with 64 maximum */
		skb_trim(skb, min_t(uint8_t, cfd->len, 64));
	}
	...
	// printk(KERN_ERR "russell--j1939_can_recv----------4-\n");
	if (can_type == 1)
	{
	...
	{
		skcb->priority = (cfd->can_id >> 26) & 0x7;
		skcb->addr.sa = cfd->can_id;
		skcb->addr.pgn = (cfd->can_id >> 8) & J1939_PGN_MAX;
	}
	// printk(KERN_ERR "russell--j1939_can_recv----------1-\n");
	...
	// printk(KERN_ERR "russell--j1939_can_recv----------2-\n");
	...
	{
		if (j1939_tp_recv_fd(priv, skb))
			goto done;
	}

	// printk(KERN_ERR "russell--j1939_can_recv----------3-\n");
...
int j1939_send_one(struct j1939_priv *priv, struct sk_buff *skb)
{
	...
	struct can_frame *cf;
	printk(KERN_ERR "russell-j1939_send_one-----------\n");
	/* apply sanity checks */
	if (j1939_pgn_is_pdu1(skcb->addr.pgn))
...
int j1939_send_one_fd(struct j1939_priv *priv, struct sk_buff *skb)
{
	int ret, dlc;
	canid_t canid;

	struct j1939_sk_buff_cb *skcb = j1939_skb_to_cb(skb);
	struct canfd_frame *cf; // 使用canfd_frame
	printk(KERN_ERR "russell-j1939_send_one_fd-----------\n");
	...
	if (unlikely(ret))
		goto failed;
	dlc = skb->len;
	// printk(KERN_ERR "russell-skb->len-------%d----\n",skb->len);
	/* re-claim the CAN_HDR from the SKB */
	cf = skb_push(skb, J1939_CANFD_HDR); // 修改为canfd_frame

	/* initialize header structure */
	memset(cf, 0, J1939_CANFD_HDR); // 修改为canfd_frame

	/* make it a full can frame again */
	skb_put(skb, J1939_CANFD_FTR + (64 - dlc));

	canid = CAN_EFF_FLAG | (skcb->priority << 26) | (skcb->addr.pgn << 8) |
		skcb->addr.sa;
	if (j1939_pgn_is_pdu1(skcb->addr.pgn))
		canid |= skcb->addr.da << 8;
	...
	/* 适应CAN FD的处理 */
	cf->flags |= CANFD_BRS; // CAN FD数据位速率切换
	...
	// /* 确保SKB有足够的头部空间 */
	// if (unlikely(skb_headroom(skb) < sizeof(struct canfd_frame))) {
	// 	pr_err("j1939: insufficient headroom++%d+\n",
	// 	       skb_headroom(skb));
	// 	// ret = -ENOMEM;
	// 	// goto failed;
	// }
	...
	ret = can_send(skb, 1);
    // printk(KERN_ERR "russell: can_send ret=%d\n", ret);
